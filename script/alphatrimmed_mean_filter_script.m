...
d = input('Masukkan d: ', 's');
d = str2double(d);

if isnan(d) || floor(d) ~= d || d < 0
    error('d tidak valid!');
elseif mod(d, 2) ~= 0
    error('d harus bilangan genap!');
else
    d = int32(d);
end


[noisy_I, new_I] = contraharmonic_mean_filter(I, type, density, m, n, Q);
...
figure; imshow(new_I); title('Citra Hasil Penapis Rerata Alphatrimmed');
figure; imhist(new_I); title('Histogram Citra Hasil Penapis Rerata Alphatrimmed');