function [I_d, result] = geometric_mean_filter(I, type, density, m, n)
    if (type == "salt&pepper")
    ...
    for k = 1 : C
        for i = 1 : M - X + 1
            for j = 1 : N - Y + 1
                ...
                for u = i : (i + X-1)
                    for v = j : (j + Y-1)
                        new_middlevalue = new_middlevalue * double(I_d(u, v, k));
                    end
                end
                
                new_middlevalue = nthroot(double(new_middlevalue), double(X*Y));
                new_middlevalue = floor(new_middlevalue);
