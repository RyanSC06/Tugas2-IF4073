function [S1, H, S2, result] = INRF(f, R, U0, V0)
    [M, N, C] = size(f);
    f = im2double(f);
    ...
    % STEP 2: Bangkitkan fungsi penapis INPF (H)
    ...
    % Membuat penapis yang hanya berisi 1 atau 0 tergantung D dan R
    H1 = double(D1 <= R);
    % Balikkan INPF menjadi INRF
    H1 = 1 - H1;
